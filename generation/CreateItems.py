...
def build_item_pool_dict(world):
    ...
    if world.options.master_keys != OraclesMasterKeys.option_disabled:
        for small_key_name in ITEM_GROUPS["Master Keys"]:
            if (
                world.options.secret_locations
...
def location_is_active(world, location_name, location_data):
    ...
    if location_name in RUPEE_OLD_MAN_LOCATIONS:
        return (
            world.options.shuffle_old_men
            == OraclesOldMenShuffle.option_turn_into_locations
        )

    if "dungeon" in location_data:
        if location_data["dungeon"] == 11:
            return (
                world.options.linked_heros_cave
                != OracleOfAgesLinkedHerosCave.option_disabled
            )
        if (
            location_data["region_id"]
            == "d" + str(location_data["dungeon"]) + " miniboss"
        ):
